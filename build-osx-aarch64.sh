...
APPBASE="build/macos-aarch64/Darkan.app"

build() {
    echo Launcher sha256sum
    shasum -a 256 build/libs/Darkan.jar
    ...
    cp native/build-aarch64/src/Darkan $APPBASE/Contents/MacOS/
    cp build/libs/Darkan.jar $APPBASE/Contents/Resources/
    ...
    otool -l $APPBASE/Contents/MacOS/Darkan
}
...
dmg() {
    ...
    mv Darkan\ *.dmg Darkan-aarch64.dmg

    # dump for CI
    hdiutil imageinfo Darkan-aarch64.dmg

    if ! hdiutil imageinfo Darkan-aarch64.dmg | grep -q "Format: ULFO" ; then
        echo Format of dmg is not ULFO
        exit 1
    fi

    if ! hdiutil imageinfo Darkan-aarch64.dmg | grep -q "Apple_HFS" ; then
        echo Filesystem of dmg is not Apple_HFS
        exit 1
    fi

    # Notarize app
    if xcrun notarytool submit Darkan-aarch64.dmg --wait --keychain-profile "AC_PASSWORD" ; then
        xcrun stapler staple Darkan-aarch64.dmg
    fi
}
